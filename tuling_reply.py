...
def tuling_reply_chat(msg):
    if msg['ToUserName'] != chatroom_name:
        return
    ...
    # print(msg)
    defaultReply = 'I received: ' + msg['Text']
    reply = u'[李狗剩]%s' % (get_response(msg['Text']))
...
# 为了让实验过程更加方便（修改程序不用多次扫码），使用热启动
game_chengyu.init()
itchat.auto_login(enableCmdQR=2)
# itchat.auto_login(hotReload=True, enableCmdQR=2)

# 搜索群聊
chatroom = itchat.search_chatrooms(name='聪明伶俐小公举')[0]
chatroom_name = chatroom['UserName']  # 聪明伶俐小公举

# 机器人进入群聊提示
# itchat.send_msg(msg='李狗剩穿着黑丝，推开了门', toUserName=chatroom_name)
print('Username Of Chatroom: {}'.format(chatroom_name))
